...
from torch.ao.quantization.quantizer.openvino_quantizer import (
...
class OpenVINOQuantizerTestCase(QuantizationTestCase):
    def _test_quantizer(
        self,
        model,
        example_inputs,
...
class TestQuantizeOpenVINOQUANTIZER(OpenVINOQuantizerTestCase):
    def test_conv2d(self):
        """
        Test pattern of single conv2d with OpenVINOQuantizer.
        """
        with torch.no_grad():
    ...
    def test_conv2d_unary(self):
        """
        Test pattern of conv2d with unary post ops (such as relu, hardtanh, hardswish, relu6) with OpenVINOQuantizer.
        """
        unary_map = {
    ...
    def test_conv2d_binary(self):
        """
        Test pattern of conv2d with binary post ops (such as add) with OpenVINOQuantizer.
    ...
    def test_conv2d_binary_unary(self):
        """
        Test pattern of conv2d with binary + unary post ops (such as add + relu) with OpenVINOQuantizer.
    ...
    def test_conv2d_serials_binary_unary(self):
        """
        Test pattern of 2 following up conv2d add relu with OpenVINOQuantizer.
        """
        with torch.no_grad():
    ...
    def test_linear(self):
        """
        Test pattern of single linear with OpenVINOQuantizer.
        """
        with torch.no_grad():
    ...
    def _test_linear_unary_helper(
        ...
        """
        Test pattern of linear with unary post ops (e.g. relu) with OpenVINOQuantizer.
        """
        use_bias_list = [True, False]
    ...
    def test_linear_binary(self):
        """
        Test pattern of linear with binary post ops (such as add) with OpenVINOQuantizer.
        Currently, only add as binary post op is supported.
        """
        ...
        quantizer = OpenVINOQuantizer()
        quantize_per_tensor_op = torch.ops.quantized_decomposed.quantize_per_tensor.default
        dequantize_per_tensor_op = torch.ops.quantized_decomposed.dequantize_per_tensor.default
    ...
    def test_linear_binary_unary(self):
        """
        Test pattern of linear with binary + unary post ops (such as add + relu) with OpenVINOQuantizer.
        Currently, only add as binary post op and relu as unary post op are supported.
        """
        linear_pos_list = [NodePosType.left, NodePosType.right, NodePosType.both]
        # TODO test for inplace add after refactoring of export_for_training
    ...
    def test_linear_binary_unary_serials(self):
        """
        Test pattern of 2 following up linear add relu with OpenVINOQuantizer.
        """
        with torch.no_grad():
    ...
    def test_filter_conv2d_recipe(self):
        """
        Test removing conv2d from default recipe of OpenVINOQuantizer.
        """
        with torch.no_grad():
            m = TestHelperModules.Conv2dUnaryModule(torch.nn.ReLU(inplace=False)).eval()
            example_inputs = (torch.randn(2, 3, 16, 16),)
            quantizer = OpenVINOQuantizer(ignored_scope=nncf.IgnoredScope(type=["conv2d"]))
    ...
    def test_attention_block(self):
        """
        Test pattern of Attention like Block with OpenVINOQuantizer.
        """
        with torch.no_grad():
